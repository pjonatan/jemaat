...
public class Minat extends JFrame {
	...
	private String column[] = {"Index","Minat"};	
	
	Minat()
	{
      setTitle("Daftar minat");
      setLayout(null);
      addWidgets();
      addMinat();
      addTable();
	...
	public void Simpan1()
	{
		...
		if (dialogButton == JOptionPane.YES_OPTION) {
			...
			try{
			    ...
			    ps = con.prepareStatement("insert into minat(minat)values(?)");
		...
		{
		   lb1 = new JLabel("Tambah minat", SwingConstants.CENTER);
		...
		public void addMinat()
		{
		   lb11 = new JLabel("Edit minat", SwingConstants.CENTER);
	...
	public void addTable()
	{
      ...
      try{
		...
		rs= st.executeQuery("SELECT COUNT(*) FROM minat");
        while (rs.next()){
            rows = rs.getInt(1);
        }
		rs = st.executeQuery("select * from minat");
    ...
    public void Ubah_Hapus(String s)
    {
		  ...
		  try{
				Connection con=DB.getConnection();
				ps = con.prepareStatement("select * from minat where m_id=?");
				ps.setString(1, s);
				rs=ps.executeQuery();
			    rs.next();
    ...
    public void Ubah1()
    {
			int tanya = JOptionPane.showConfirmDialog (null, "Mau ubah minat?",null, JOptionPane.YES_NO_OPTION);
  			if(tanya ==JOptionPane.YES_OPTION)
  			{
  				...
  				try{
  					...
  					Connection con=DB.getConnection();
  					ps = con.prepareStatement("update minat set minat=? where m_id=" + gid);
  					ps.setString(1, tf12.getText());  					
  				    ps.executeUpdate();
  				    con.close();
  				}catch(Exception e){System.out.println(e);} 					
  				JOptionPane.showMessageDialog(this, "Minat sudah diubah!!");
  			}    	
    }
    
    public void Hapus1()
    {
		int tanya = JOptionPane.showConfirmDialog (null, "Mau hapus Minat?",null, JOptionPane.YES_NO_OPTION);
			if(tanya ==JOptionPane.YES_OPTION)
			{
				PreparedStatement ps =null;
				try{
					Connection con=DB.getConnection();
					ps = con.prepareStatement("delete from minat  where m_id=?");
					ps.setString(1, tf11.getText());
				    ps.executeUpdate();
				    con.close();
				}catch(Exception e){System.out.println(e);} 					
				JOptionPane.showMessageDialog(this, "Minat sudah dihapus!!");
			}    	    	
    }
}
